...
serology <- serology %>% mutate(time_point = ifelse(days_after_vaccination > 180, 'T4', time_point))
...
```{r Estimate NT50 decay rate post-second dose of vaccine, results = 'hide'}
...
```{r Estimate IgG decay rate post-second dose of vaccine, results='hide'}
# for IGG
...
```{r Plot IgG/NT50 dynamics by months, results = 'hide'}
...
mean_points_nt50 <- serology %>%
  ...
  annotate(geom = "text", x = 8, y = 3, label = expression("t"[1 / 2] * " = " * "45 days"), size = 4) +
  theme(legend.position = "top", legend.title = element_blank()) +
  geom_circle(aes(x0=13,y0=0,r=0.25), inherit.aes = FALSE, size = .5) + 
  annotate(geom = "text", x = 10.5, y = 0.5, label = "non-neutralizing individuals", size = 3)
...
mean_points_igg <- serology %>%
  ...
  scale_x_discrete(limits = factor(seq(0, 6, 0.5)), labels = sapply(1:length(seq(0, 6, 0.5)), function(i) ifelse(i %% 2 == 0, "", seq(0, 6, 0.5)[i]))) +
  annotate(geom = "text", x = 8, y = 4, label = expression("t"[1 / 2] * " = " * "55 days"), size = 4) +
  theme(legend.position = "top", legend.title = element_blank()) +
...
```{r Get the IgG boundary value for neutralizing-after-6mths-vs-others, results = 'hide'}
non_neutr_n <-
  serology %>%
  filter(!is.na(days_after_vaccination)) %>%
  ungroup() %>%
...
p1 <- serology %>%
  ...
  mutate(
    ...
    neutralization = as.factor(case_when(
      patid %in% c(patients_wo_neutralization_at_all, patients_wo_neutralization_after_halfayear) ~ glue("non-neutralizing\nafter 6 months\nn = {non_neutr_n}"),
      TRUE ~ glue("neutralizing\nafter 6 months\nn = {neutr_n}")
    ))
  ) %>%
...
p2 <- ggplot(df, aes(x = igg_antispike, y = as.numeric(neutralization_status) - 1)) +
  ...
  annotate("text", x = log10(boundary_value) - 0.3, y = 0.5, label = glue("{round(boundary_value)} BAU/ml"), size = 3) +
  annotate("text", x = 4.8, y = 0.75, label = "5-fold CV:", size = 3) +
  annotate("text",
    x = 4.4, y = 0.75,
    label = TeX(paste0("\\textit{sensitivity} = ", sens_spec$sens_mean, "\\pm{}", sens_spec$sens_sd, " %")), size = 3
  ) +
  annotate("text",
    x = 4, y = 0.75,
    label = TeX(paste0("\\textit{specificity} = ", sens_spec$spec_mean, "\\pm{}", sens_spec$spec_sd, " %")), size = 3
  ) +
  theme(
...
### fig1
```{r fig1, fig.width=7.8, fig.height=7.8, results='hide'}
PLOT_neutralizing_status_logreg <- ggarrange(p1, p2, common.legend = TRUE, legend = "right", widths = c(1.55, 1), labels = c("E", "F"), label.x = c(0, -0.025), font.label = list(size = 10))

fig1 <- 
  ggarrange(PLOT_serology_dynamics, PLOT_neutralizing_status_logreg,
  ncol = 1,
  heights = c(2, 1)
)
...
```{r Look at class separation across time points: suppl_neutralizing_status_logreg, fig.width=7, fig.height=6, results='hide'}
...
#ggsave(path = "analysis/plots/vaccination_study/paper/",
#       filename = "suppl_neutralizing_status_logreg.jpg",
#       plot = PLOT,
#       width=7, height=6,
#       device = "jpg", dpi=300)
...
```{r Make separate table with cellular response data, results = 'hide'}
...
```{r Check NT-status association significance in the full model including recovery status instead of igg and another grouping, results = 'hide'}
...
```{r IgG levels at T0-T2-T4: fig2, fig.width=7, fig.height=7, results='hide'}
...
p1_igg <- ggstatsplot::ggcoefstats(
  ...
  theme_pubr(base_size = 11) +
  xlab("% conditional Var ") + ggtitle(TeX("anti-S $IgG_{T0:T4}$ ~")) + scale_x_continuous(labels = seq(0, 1, 0.25) * 100)
...
```{r suppl_no_age_bias_check, fig.width=4, fig.height=4, results='hide'}
...
#ggsave(path = "analysis/plots/vaccination_study/paper/",
#       filename = "suppl_no_age_bias_check.jpg",
...
```{r Plot NT50 across time model, results = 'hide'}
p1_nt50 <- ggstatsplot::ggcoefstats(
  ...
  relabel_predictors(sex = "sex (male)", smoking = "smoking", age = "age\n(scaled)", BMI = "BMI\n(scaled)") +
  theme_pubr(base_size = 11) +
  xlab("% conditional Var ") + ggtitle(TeX("$NT50_{T0:T4}$ ~")) + scale_x_continuous(labels = seq(0, 1, 0.25) * 100)
...
nt50_vs_igg <- ggarrange(serology_correlation_plots[[1]],
  ...
  labels = paste0("T", 0:4), label.x = 0.05, font.label = list(size = 10, face = c("bold.italic"))
)
nt50_vs_igg <- annotate_figure(nt50_vs_igg,
  left = text_grob(ylab, rot = 90, size = 10),
  bottom = text_grob(xlab, size = 10)
)
```


```{r fig3, fig.width=7.5, fig.height=10, results='hide'}
fig3 <- ggarrange(nt50_vs_igg, plots, widths = c(2, 3.75), labels = "A")
...
```{r Correlate serological and cellular response on the last time point, results = 'hide'}
...
```{r suppl_ifn_associations, fig.width=6, fig.height=3, results='hide'}
dftmp <- df %>%
...
ag1_mod <-
  ...
  theme_pubr(base_size = 12) +
  xlab("regression coefficient") +
  ggtitle(label = element_blank(), subtitle = TeX("$_{log_{10}}$ QuantiFERON\nSARS-CoV-2 Ag1 ~"))
...
#ggsave(path = "analysis/plots/vaccination_study/paper/",
#       filename = "suppl_ifn_associations.jpg",
#       plot = PLOT,
#       width=6, height=3,
#       device = "jpg", dpi=300)
...
```{r IFN comparison between statuses: fig4, fig.width=7.5, fig.height=12, results='hide'}
df <- cellular_response %>%
  ungroup() %>%
  mutate(
    neutralization_status = as.factor(case_when(
...
ifn_neutralization_ag1 <- df %>%
  ...
  stat_compare_means(label.y.npc = 0.1, label.x = 1.5, size = 3) +
  geom_violin(size = 0.5) +
  geom_beeswarm(size = 2) +
  scale_color_manual(values = c("#00AFBB", "#E7B800", "#FC4E07", "#999999")) +
  geom_boxplot(width = 0.1, color = "black", alpha = 0.3) +
