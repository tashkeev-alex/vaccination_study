...
```{r Read data}
recovered_patients <- read_lines("data/processed/vaccination_study_public/recovered_patients.txt")

quantiferon <- read_csv("data/processed/vaccination_study_public/quantiferon.csv") %>% suppressMessagesAndWarnings()

igg <- read_csv("data/processed/vaccination_study_public/igg.csv") %>% suppressMessagesAndWarnings()

serology <- read_csv("data/processed/vaccination_study_public/serology.csv") %>% suppressMessagesAndWarnings()

clinical_data <- read_csv("data/processed/vaccination_study_public/clinical_data.csv") %>% suppressMessagesAndWarnings()
```


```{r Estimate NT50 decay rate post-second dose of vaccine}
decay_estimation_df <- serology %>% filter(time_point != "T1")
decay_estimation_df <- decay_estimation_df %>%
  group_by(patid) %>%
  mutate(days_after_2nd_shot = days_after_vaccination - days_after_vaccination[time_point == "T2"]) %>%
...
group <- ifelse(decay_estimation_df_nt50$status == "recovered", 1, 0)

# X is the design matrix for the fixed effect; group is a binary variable for recovered vs naive data, interaction with day accounts for differences in the decay rate
# Z is the design matrix for the random effects for intercept and slope
fit_linear_different_rate <- lmec(
  yL = y,
  cens = cens,
...
pchisq(-2 * (fit_linear_same_rate$loglik - fit_linear_different_rate$loglik),
  df = length(fit_linear_different_rate$beta) - length(fit_linear_same_rate$beta),
  lower.tail = FALSE
)
# 0.44 => no, so decay is not detected as different for recovered and naive if using single-slope model
```


```{r Estimate IgG decay rate post-second dose of vaccine}
# for IGG
...
intercept_matrix <- matrix(rep(1, length(y)), ncol = 1)
group <- ifelse(decay_estimation_df_igg$status == "recovered", 1, 0)
# X is the design matrix for the fixed effect; group is a binary variable for recovered vs naive data, interaction with day accounts for differences in the decay rate
# Z is the design matrix for the random effects for intercept and slope
fit_linear_different_rate <- lmec(
  yL = y,
...
# 0.06 => no, so decay is not detected as different for recovered and naive if using single-slope model
```


```{r Plot IgG/NT50 dynamics by months, fig.width=12, fig.height=8}
all_points_nt50 <- serology %>%
  filter(!is.na(days_after_vaccination)) %>%
  mutate(status = case_when(
    status == "naive" ~ glue("naive\nn = {nrow(filter(serology, !is.na(days_after_vaccination), time_point == 'T1', status == 'naive'))}"),
    status == "recovered" ~ glue("recovered\nn = {nrow(filter(serology, !is.na(days_after_vaccination), time_point == 'T1', status == 'recovered'))}")
  )) %>%
  ggplot(aes(
...
mean_points_igg <- serology %>%
  ...
  scale_x_discrete(limits = factor(seq(0, 6, 0.5)), labels = sapply(1:length(seq(0, 6, 0.5)), function(i) ifelse(i %% 2 == 0, "", seq(0, 6, 0.5)[i]))) +
  annotate(geom = "text", x = 8, y = 4, label = expression("t"[1 / 2] * " = " * "95 days"), size = 6) +
  theme(legend.position = "top", legend.title = element_blank()) +
  ylab(TeX("$_{log_{10}}$ anti-S IgG $_{(BAU/ml)}$"))


PLOT_serology_dynamics <-
  ggarrange(mean_points_nt50, all_points_nt50,
    mean_points_igg, all_points_igg,
...
```{r Get the IgG boundary value for neutralizing-after-6mths-vs-others, fig.width=8, fig.height=4}
non_neutr_n <-
  serology %>%
  filter(!is.na(days_after_vaccination)) %>%
  ungroup() %>%
...
class_separation <- vector("list", 5) %>% setNames(paste0("T", 1:5))
for (TIME_POINT in c(paste0("T", 1:5))) {
  df <-
    serology %>%
    filter(!is.na(days_after_vaccination)) %>%
    ungroup() %>%
...
```{r Make separate table with cellular response data}
...
```{r Check NT-status association significance in the full model including recovery status instead of igg and another grouping, fig.width=7, fig.height=4}
...
model_plot_recovery <-
  ...
  relabel_predictors(c(sexM = "sex (male)", smoking1 = "smoking", age = "age (scaled)", BMI = "BMI (scaled)", recovery_statusrecovered = "recovery")) +
...
```{r IgG levels at T1-T3-T5: fig2, fig.width=8, fig.height=8}
model_plots <- vector("list", 5) %>% setNames(paste0("T", 1:5))
TIME_POINT_LABEL <- c("before vaccination", "after 1 dose", "after 2 doses", "T4", "after 6 months") %>% setNames(paste0("T", 1:5))
for (TIME_POINT in paste0("T", 1:5)) {
  dftmp <- df %>%
    filter(time_point == TIME_POINT) %>%
  ...
  model_plots[[TIME_POINT]] <-
    ...
    relabel_predictors(c(sexM = "sex (male)", smoking1 = "smoking", age = "age (scaled)", BMI = "BMI (scaled)", igg = glue("anti-S IgG at {TIME_POINT}\n(scaled)"))) +
    theme_pubr(base_size = 13) +
    xlab("regression coefficient") +
...
igg_multivariate_md <- lm(cbind(T1, T2, T3, T4, T5) ~ sex + smoking + age + BMI, data = dftmp)
summary(manova(igg_multivariate_md))
```


```{r Plot IgG across time model, fig.width=12, fig.height=6}
p1_igg <- ggstatsplot::ggcoefstats(
  ...
  theme_pubr(base_size = 15) +
  xlab("% conditional variance ") + ggtitle(TeX("anti-S $IgG_{T1:T5}$ ~")) + scale_x_continuous(labels = seq(0, 1, 0.25) * 100)

p2_igg <- tidy(igg_multivariate_md) %>%
  mutate(model = as.factor(response)) %>%
  dwplot(
    dot_args = list(size = 2, color = "black"),
    vars_order = c("sexM", "age", "BMI", "smoking"),
    whisker_args = list(aes(color = model, group = model), size = 2, color = rep(viridis_pal(option = "viridis")(5), 4)),
    vline = geom_vline(xintercept = 0, colour = "grey50", linetype = 2), model_order = c("T1", "T2", "T3", "T4", "T5")
  ) %>%
  relabel_predictors(c(sex = "sex (male)", smoking = "smoking", age = "age\n(scaled)", BMI = "BMI\n(scaled)")) +
  theme_pubr(base_size = 15) +
...
nt50_multivariate_md <- lm(cbind(T1, T2, T3, T4, T5) ~ sex + smoking + age + BMI, data = dftmp)
summary(manova(nt50_multivariate_md))
```

Age is kinda significant on the T1 for both IgG and NT50 - check whether there is no bias according to the status
...
p1_nt50 <- ggstatsplot::ggcoefstats(
  ...
  relabel_predictors(sex = "sex (male)", smoking = "smoking", age = "age\n(scaled)", BMI = "BMI\n(scaled)") +
  theme_pubr(base_size = 15) +
  xlab("% conditional variance ") + ggtitle(TeX("$NT50_{T1:T5}$ ~")) + scale_x_continuous(labels = seq(0, 1, 0.25) * 100)
...
```{r}
serology_correlation_plots <- vector("list", 5) %>% setNames(paste0("T", 1:5))
...
for (TIME_POINT in paste0("T", 1:5)) {
  ...
  pmain <- ggscatter(nt50_vs_igg,
    x = "neutralization_titre", y = "igg_antispike_log10",
    color = "status", alpha = 0.5, shape = 10, palette = c("#56B4E9", "#D55E00"),
  ...
  xdens <- axis_canvas(pmain, axis = "x") +
    ...
    fill_palette(palette = c("#56B4E9", "#D55E00"))
...
nt50_vs_igg <- ggarrange(serology_correlation_plots[[1]],
  ...
  labels = paste0("T", 1:5), label.x = 0.05, font.label = list(size = 11, face = c("bold.italic"))
)
...
```{r Correlate serological and cellular response on the last time point, fig.width=10, fig.height=10}
pmain <- function(XAXIS, YAXIS, label_xaxis, label_yaxis, legend = FALSE) {
  p <- ggscatter(cellular_response,
    x = XAXIS, y = YAXIS,
    alpha = 0.75,
    color = "status", shape = 10, palette = c("#56B4E9", "#D55E00"),
  ...
  if (legend == TRUE) p <- p + theme(legend.position = "top")
  if (label_xaxis) {
...
xdens <- function(PMAIN, XAXIS) {
  axis_canvas(PMAIN, axis = "x") +
    ...
    fill_palette(palette = c("#56B4E9", "#D55E00")) #+theme(plot.margin=unit(c(-0.5,0,-0.5,-0.5),"cm"))
}
ydens <- function(PMAIN, YAXIS) {
  axis_canvas(PMAIN, axis = "y", coord_flip = TRUE) +
    ...
    coord_flip() +
    fill_palette(palette = c("#56B4E9", "#D55E00")) #+theme(plot.margin=unit(c(0,-0.5,-0.5,0),"cm"))
}
...
```{r suppl_ifn_associations, fig.width=6, fig.height=3}
dftmp <- df %>%
...
df <- cellular_response %>%
  ...
  mutate(
    ...
    neutr = case_when(
      patid %in% patients_wo_neutralization_at_all ~ glue("NO at 1 and 6 months\ntime points\n\nn = {length(unique(patients_wo_neutralization_at_all))}"),
      patid %in% patients_wo_neutralization_after_1st_dose & !patid %in% patients_wo_neutralization_after_halfayear ~ glue("NO at 1 month\ntime point\n\nn = {length(unique(patients_wo_neutralization_after_1st_dose[!patients_wo_neutralization_after_1st_dose %in% patients_wo_neutralization_after_halfayear]))}"),
      patid %in% patients_wo_neutralization_after_halfayear & !patid %in% patients_wo_neutralization_after_1st_dose ~ glue("NO at 6 months \ntime point\n\nn = {length(unique(patients_wo_neutralization_after_halfayear[!patients_wo_neutralization_after_halfayear %in% patients_wo_neutralization_after_1st_dose]))}"),
      TRUE ~ glue("YES\nsince 1 month\n\nn = {length(unique(pull(filter(serology, !is.na(days_after_vaccination), !patid %in% c(patients_wo_neutralization_after_1st_dose, patients_wo_neutralization_after_halfayear)), patid)))}")
    )
  )
...
fig4 <- plot_grid(p1,
  ...
  ncol = 1, rel_heights = c(2, 1, 1), labels = c("A", "B", "C")
)
